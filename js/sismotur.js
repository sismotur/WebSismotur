...
function parallaxBanner() {
  var controller = new ScrollMagic.Controller({
    globalSceneOptions: {
      triggerHook: "onLeave",
      duration: $("#parallaxTrigger").height()
    }
  });

  var scene2 = new ScrollMagic.Scene({ triggerElement: "#parallaxTrigger" })
    .setTween(
      TweenMax.fromTo("#fondo-azul", 1, { autoAlpha: 0.1 }, { autoAlpha: 1 })
    )
    .addTo(controller);
...
function opaqueMenu() {
  var controller = new ScrollMagic.Controller({
    globalSceneOptions: { triggerHook: 0.1, duration: 0 }
  });
...
function doOpenPassportInventrip() {
  ...
  var lang = getCookie("lang");
  if (lang == "") {
    lang = getDefaultNavigatorLanguage();
  }
  console.log(lang);
  ...
  switch (lang) {
    case "es":
      url =
        "https://www.flipsnack.com/inventrip/pasaporte_inventrip_es-fcpiwkkzy.html";
      break;
    case "pt":
    ...
    case "fr":
      url =
        "https://www.flipsnack.com/inventrip/passeport_inventrip_fr-fd1jlhhv0.html";
      break;
    case "ca":
